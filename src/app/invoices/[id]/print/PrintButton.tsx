...
import CreditNoteModal from "@/app/(app)/invoices/CreditNoteModal";
import type { Invoice } from "@/lib/types";

export default function PrintButton({ invoice }: { invoice: Invoice }) {
  const [returnOpen, setReturnOpen] = useState(false);
  const { id, customer_phone } = invoice;
  ...
  return (
    <>
      <div className="no-print" style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap", margin: "18px 0" }}>
        ...
        <button className="btn btn-red" onClick={() => setReturnOpen(true)}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>
          Return
        </button>
      ...
      {returnOpen && <CreditNoteModal invoice={invoice} onClose={() => setReturnOpen(false)} />}
    </>
  );
}
